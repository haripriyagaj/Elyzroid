...
const Toolkit = ({ modules, onToggle, toolkitLogs, addToolkitLog = () => {} }) => {
  ...
  const handleToggleNotification = async (enabled) => {
    ...
    if (enabled) {
      ...
      setNotifAccessEnabled(access.enabled);
      if (!access.enabled) {
        addToolkitLog(`<span style="color:var(--ely-yellow)">[WARN]</span> Notification access not granted. Tap "Grant Notification Access" to enable system monitoring.`);
      }
